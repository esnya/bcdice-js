...
class DiceBotResolver
  ...
  def self.load(filename)
    ...
    if (@@mode == 'dynamic-require')
      ...
      `var result = null;`
      `var error = null;`
      `try { result = require('./diceBot/' + #{filename}); } catch (e) { error = e; result = null; }`
    
      debug("require", `result ? 'ok' : 'error'`)
      debug(`error`)
  ...
  def self.loadAsync(filename)
    ...
    if `#{filename} in #{@@cache}` then
      debug('chache hit', filename)
      `Promise.resolve(#{@@cache}[filename])`
    else
      debug('chache miss', filename)
      `#{@@loader}(filename)`
    end
  end
